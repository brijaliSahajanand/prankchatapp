...
import ru.fotostrana.socketapp.VPN.Sample_Connection;
import ru.fotostrana.socketapp.activity.ComingsoonActivity;
import ru.fotostrana.socketapp.activity.First_Activity;
import ru.fotostrana.socketapp.constant.APIClient;
import ru.fotostrana.socketapp.constant.Preference;
import ru.fotostrana.socketapp.constant.Utils;
import ru.fotostrana.socketapp.retrofit.APIInterface;
import ru.fotostrana.socketapp.retrofit.Example;
import ru.fotostrana.socketapp.retrofit.Pro_IPModel;
import ru.fotostrana.socketapp.retrofit.TraficLimitResponse;
...
import static ru.fotostrana.socketapp.constant.Utils.isdebuggle;
import static ru.fotostrana.socketapp.constant.Utils.warningdialog;
...
public class Splash_Activity extends BaseActivity {
    ...
    private void subScribePushChannel() {
        ...
        try {
            FirebaseMessaging.getInstance().subscribeToTopic("PrankChat_BZ1")
    ...
    private void CallIpApi() {
        ...
        Call<Pro_IPModel> call = apiInterface.getIp();
        call.enqueue(new Callback<Pro_IPModel>() {
            ...
            public void onResponse(Call<Pro_IPModel> call, Response<Pro_IPModel> response) {
                if (response.isSuccessful()) {
                    ...
                    Log.d("publicip", "onResponse: ip = " + ip);
                } else {
                    get_ip = false;
...
//                Log.d("publicip", "onfailure: failed");
    ...
    private void CallUserDetails() {
        ...
        APIInterface apiInterface = retrofit.create(APIInterface.class);
        String url = "";
        if (get_ip && !ip.isEmpty()) {
            url = ip;
        } else {
            url = "?fields=61439";
        }
        //Log.d("publicip", "onResponse: url = " + url);
        Call<Pro_IPModel> call = apiInterface.getUserDetails(url);
        call.enqueue(new Callback<Pro_IPModel>() {
            @Override
            public void onResponse(Call<Pro_IPModel> call, Response<Pro_IPModel> response) {
                if (response.isSuccessful()) {
              ...
              /*  Log.d("publicip", "Region = " + Region);
                Log.d("publicip", "Country = " + Country);
                Log.d("publicip", "City = " + City);
                Log.d("publicip", "onResponse: getQuery = " + response.body().getQuery());*/
                } else {
                    //Log.d("publicip", "onResponse: failed");
                }
                setting_api();
